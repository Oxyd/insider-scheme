...
TEST_F(port_fixture, read_one_character_from_port) {
  ...
  EXPECT_EQ(p->peek_character()->value(), 'a');
  EXPECT_EQ(p->read_character()->value(), 'a');
  EXPECT_FALSE(p->peek_character());
}
...
TEST_F(port_fixture, read_sequence_of_characters) {
  auto p = make_string_input_port("abc");
  EXPECT_EQ(p->read_character()->value(), 'a');
  EXPECT_EQ(p->read_character()->value(), 'b');
  EXPECT_EQ(p->read_character()->value(), 'c');
  EXPECT_FALSE(p->read_character());
}

TEST_F(port_fixture, read_non_ascii_character) {
  auto p = make_string_input_port(u8"á");
  EXPECT_EQ(p->peek_character()->value(), U'á');
  EXPECT_EQ(p->read_character()->value(), U'á');
  EXPECT_FALSE(p->peek_character());
}

TEST_F(port_fixture, read_sequence_of_non_ascii_characters) {
  auto p = make_string_input_port(u8"příšerně žluťoučký kůň");
  EXPECT_EQ(p->read_character()->value(), U'p');
  EXPECT_EQ(p->read_character()->value(), U'ř');
  EXPECT_EQ(p->read_character()->value(), U'í');
  EXPECT_EQ(p->read_character()->value(), U'š');
  EXPECT_EQ(p->read_character()->value(), U'e');
  EXPECT_EQ(p->read_character()->value(), U'r');
  EXPECT_EQ(p->read_character()->value(), U'n');
  EXPECT_EQ(p->read_character()->value(), U'ě');
}

TEST_F(port_fixture, peek_and_read_put_back_character) {
  auto p = make_string_input_port("a");
  p->put_back(character{'b'});
  EXPECT_EQ(p->peek_character()->value(), 'b');
  EXPECT_EQ(p->read_character()->value(), 'b');
  EXPECT_EQ(p->read_character()->value(), 'a');
  EXPECT_FALSE(p->read_character());
}
...
TEST_F(port_fixture, rewind) {
  ...
  p->read_character();
  p->read_character();
  EXPECT_EQ(p->peek_character()->value(), 'c');
  p->rewind();
  EXPECT_EQ(p->peek_character()->value(), 'a');
  EXPECT_EQ(p->read_character()->value(), 'a');
  EXPECT_EQ(p->read_character()->value(), 'b');
