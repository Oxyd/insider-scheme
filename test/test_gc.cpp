...
TEST_F(gc, tracker_tracks_local_ptrs) {
...
TEST_F(gc, tracker_can_track_multiple_ptrs) {
  ptr<string> a = make<string>(ctx, "foo");
  ptr<string> b = make<string>(ctx, "bar");
  tracker t{ctx, a, b};
  ctx.store.collect_garbage(true);
...
TEST_F(gc, tracker_can_track_vector_of_ptrs) {
  std::vector<ptr<string>> v;
  v.push_back(make<string>(ctx, "foo"));
  v.push_back(make<string>(ctx, "bar"));
  tracker t{ctx, v};
  ctx.store.collect_garbage(true);
  for (ptr<string> p : v)
...
TEST_F(gc, tracker_can_track_custom_visitable_type) {
  container c{make<string>(ctx, "foo")};
  tracker t{ctx, c};
  ctx.store.collect_garbage(true);
  EXPECT_TRUE(is_valid(c.x));
}

TEST_F(gc, tracker_can_track_vector_of_visitable_type) {
  std::vector<container> v;
  v.push_back(container{make<string>(ctx, "foo")});
  v.push_back(container{make<string>(ctx, "bar")});
  tracker t{ctx, v};
  ctx.store.collect_garbage(true);
  EXPECT_TRUE(is_valid(v[0].x));
  EXPECT_TRUE(is_valid(v[1].x));
}
