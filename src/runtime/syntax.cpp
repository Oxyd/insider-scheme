...
export_syntax(context& ctx, ptr<module_> result) {
  define_procedure<&syntax::update_and_get_expression>(ctx, "syntax-expression",
                                                       result);
  define_constant_evaluable_procedure<syntax_scopes>(ctx, "syntax-scopes",
                                                     result);
  ...
  define_constant_evaluable_procedure<free_identifier_eq>(
    ctx, "free-identifier=?", result
  );
  define_constant_evaluable_procedure<bound_identifier_eq>(
    ctx, "bound-identifier=?", result
  );
  define_constant_evaluable_procedure<syntax_location>(ctx, "syntax-location",
                                                       result);
}
