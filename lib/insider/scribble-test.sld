;; Adopted from Alex Shinn's implementation
;;
...
(define-library (insider scribble-test)
  ...
  (begin
    ...
    (define (run-tests)
      ...
      (test-scribble '((foo #R"(blah "blah" (`blah'?))")) #R"(@foo{blah "blah" (`blah'?)})")
  ...
  yada yada
   ...
   baz}")
      ...
      (test-scribble '((foo "A string escape")) #R"(@foo{A @"string" escape})")
      (test-scribble '((foo "eli@barzilay.org")) #R"(@foo{eli@"@"barzilay.org})")
      (test-scribble '((foo #R"(eli\barzilay.org)")) #R"(@foo{eli@"\\"barzilay.org})")
      (test-scribble '((foo "A { begins a block")) #R"(@foo{A @"{" begins a block})")
      (test-scribble '((C "while (*(p++)) {" "\n" #R"(     *p = '\n';)" "\n" "   }"))
                     #R"(@C{while (*(p++)) {
     *p = '@"\\"n';
   }})")
      ...
      (test-scribble '((foo "bar " (x "\\") " baz")) #R"(@foo|{bar |@x|{\}| baz}|)")
  ...
  baz}")
     ...
     blah}")
      ...
      (test-scribble '((foo "The prefix: @.")) #R"(@foo{The prefix: @"@".})")
      (test-scribble '((foo #R"(The prefix: \.)")) #R"(@foo{The prefix: @"\\".})")
      (test-scribble '((foo #R"(@x{y} --> (x "y"))")) #R"(@foo{@"@\x{y}" --> (x "y")})")

      (test-scribble '((foo "...")) "@foo|{...}|")
      (test-scribble '((foo #R"("}" follows "{")")) #R"(@foo|{"}" follows "{"}|)")
      ...
      |@bar{is}
      Life!}|")
      (test-scribble '((t "In " (i "sub\\s") " too")) #R"(@t|{In |@i|{sub|@"\\"s}| too}|)")
      ...
      (test-scribble '((foo "xyz")) #R"(@foo{x@"y"z})")
      (test-scribble '((foo "x" "y" "z")) #R"(@foo{x@|"y"|z})")
